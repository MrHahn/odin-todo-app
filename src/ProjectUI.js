    ...
    static displayTodos(project){
        ...
        todoArr.forEach((item, index) => {
            ...
            todoWrapper.addEventListener('click', () => {
                //  todoWrapper.classList.add('expanded');
                //  todoWrapper.appendChild(close);
                let todoClone = todoWrapper.cloneNode(true);
                todoClone.classList.add('expanded');
                todoClone.appendChild(close);
                todoWrapper.appendChild(todoClone);
                close.addEventListener('click', (event) => {
                    event.stopPropagation();
                    // todoWrapper.classList.remove('expanded');
                    todoClone.remove();
                })
            })
